...
class ApiClient
{
    ...
    public function getLabel($id, $send_date='')
    {
        if (is_null($id) || empty($id)) {
            throw new \InvalidArgumentException(
                'Parameter `id` must contains a data'
        ...
        return $this->client->makeRequest(sprintf('forms/backlog/%s/forms', $id), Client::METHOD_GET, ['sending-date'=>$send_date]);
    }
    ...
    /*public function getLabel($id, $send_date)
    {
        ...
        return $this->client->makeRequest(sprintf('/postoffice/1.0/settlement.offices.codesforms/%s/f103pdf', $name), Client::METHOD_GET);
    }*/
