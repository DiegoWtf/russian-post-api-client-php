...
class Client
{
    ...
    public function makeRequest($path, $method = 'GET', $parameters = array(), $headers = array(), $timeout = 40) {
        $url = $this->url . $path;
        ...
        //curl_setopt($curlHandler, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_1_0);
        //curl_setopt($curlHandler, CURLOPT_TCP_KEEPALIVE, 1);
        //curl_setopt($curlHandler, CURLOPT_TCP_KEEPIDLE, 2);
        ...
        if ((is_array($parameters) > 0 || is_string($parameters)) && in_array($method, array(self::METHOD_POST, self::METHOD_PUT, self::METHOD_DELETE))) {
